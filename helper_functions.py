...
def get_annotation_file(annotations_path):
    files = os.listdir(annotations_path)
    annot_file = pd.DataFrame([], columns=['file_name', 'xmin', 'ymin', 'xmax', 'ymax'])
    for file in files:
        name = file.replace('.txt', '.tif')
        with open(annotations_path + file, 'r') as f:
            line = f.readline()
            xmin, ymin, xmax, ymax = line.strip().split(',')
            xmin, ymin, xmax, ymax = int(xmin), int(ymin), int(xmax), int(ymax)
        annot_file.loc[len(annot_file.index)] = [name, xmin, ymin, xmax, ymax] 
    return annot_file
    ...
    with open(input_path, 'r') as f:
        ...
        for line in f:
            ...
            if filename not in all_imgs:
                ...
                img = cv2.imread('data/TrainImages/' + filename)
...
def augment(img_data, config, augment=True):
    ...
    path_images = 'data/TrainImages/'
    if not augment:
        path_images = 'data/TestImages/'

    img = cv2.imread(path_images + img_data_aug['filepath'])
