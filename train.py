...
def train_all():
    ...
    ann = get_annotation_file('data/TrainGroundTruth/')
    ann.to_csv('data2.csv', index=False)
    train_path = 'data2.csv' # Training data (annotation file)
    ...
    # train_imgs = get_data(train_path)
    print()
    ...
    classes_nums = len(classes_count)

    classifier = classifier_layer(shared_layers, roi_input, C.num_rois, nb_classes=classes_nums)
    ...
    model_classifier.compile(optimizer=optimizer_classifier, loss=[class_loss_cls, class_loss_regr(classes_nums-1)], metrics={'dense_class_{}'.format(classes_nums): 'accuracy'})
